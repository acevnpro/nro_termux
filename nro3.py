...
def apply_and_build(cfg):
    ...
    if os.path.exists(lib_dir):
        ...
        if not os.path.exists(new_driver_path) or os.path.getsize(new_driver_path) < 1024:
            try:
                p_info("Downloading MySQL driver using urllib...")
                urllib.request.urlretrieve(new_driver_url, new_driver_path)
            except Exception as e:
                p_info(f"urllib failed ({e}), falling back to wget...")
        ...
        for od in old_drivers:
            ...
            if os.path.exists(od_path):
                import shutil
                shutil.copy2(new_driver_path, od_path)
...
def manage_lemp(cfg):
    ...
    while True:
        ...
        if ch == "1":
            ...
            print(f"    • Database: {C.G}nrovip{C.E}")
            wait()
        elif ch == "4":
...
def main():
    while True:
        ...
        print(f"""{C.CY}{C.BOLD}
==========================================
        NRO VNPro_3 DÀNH RIÊNG CHO SRC_3
=========================================={C.E}
